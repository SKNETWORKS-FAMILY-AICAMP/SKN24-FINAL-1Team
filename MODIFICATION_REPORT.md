...
- 회의 목록 GET은 관리자 외에는 생성자 또는 참여자인 회의만 반환
...
- 모든 칸반 컬럼에 가장 긴 컬럼 높이를 강제로 적용해 카드가 없는 컬럼도 배경이 길게 내려갔습니다.
...
- 모든 컬럼에 `maxColumnHeight`를 넘기던 코드 제거
...
- 각 칸반 컬럼 배경 높이가 자기 카드 개수 기준으로 동작합니다.

### `frontend/hpm/src/components/project/KanbanColumn.tsx`

수정 이유:

- 컬럼 내부 스크롤 또는 공통 최소 높이 때문에 카드 개수와 배경 높이가 맞지 않았습니다.

수정 내용:

- `minHeight` prop 제거
- `getKanbanColumnHeight(tasks.length)`만으로 컬럼 높이 계산
- 본문 높이와 content 높이를 컬럼 내부 계산값으로 지정

결과:

- 진행중 1개, 완료 0개 같은 경우 각 컬럼 배경이 카드 수에 맞게 짧아집니다.
